...
OUTPUT_PATH = "f:/PharmaQMS-Vue/analysis_activation.txt"
KEYWORDS = [
    "reverse().join",
    "reverse",
    "atob",
...
def search_in_file(file_path):
    try:
        ...
        for keyword in KEYWORDS:
            # Escape keyword for regex, but allow spaces
            pattern = re.escape(keyword)
            if keyword == "reverse().join":
                pattern = r"reverse\(\)\.join"
            
            matches = [m.start() for m in re.finditer(pattern, content, re.IGNORECASE)]
            if matches:
